...
public class BlockPixelizedWithOutlineShaderGUI : ShaderGUI
{
    bool showColor, showAlpha, showPixelize, showLighting, showOutline, showCrack;
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        DrawCrackGroup(materialEditor, properties);
    ...
    public void DrawCrackGroup(MaterialEditor editor, MaterialProperty[] properties)
    {
        showCrack = EditorGUILayout.BeginFoldoutHeaderGroup(showCrack, "Crack");
        if (showCrack)
        {
            var crack = FindProperty("_Crack", properties);
            editor.RangeProperty(crack, "Crack");

            for (int i = 1; i <= 10; i++)
            {
                var tex = FindProperty($"_Crack_Tex{i}", properties);
                editor.TextureProperty(tex, $"Crack Level {i}", false);
            }
        }
        EditorGUILayout.EndFoldoutHeaderGroup();
    }
